...
class LiveMonitor {
    constructor(youtubeUrl, emailAlerts, activeMonitorsMap = null, scheduler = null) {
    ...
    _runYtdlp(args, timeout = 30000) {
        ...
        const now = Date.now();
        // Se não for força e houver cache válido, usa o cache
        if (!force && this._cachedMetadata && (now - this._metadataCacheTime) < this._metadataTTL) {
            console.log(`[${this.videoId}] 📦 Usando cache de metadados (${((now - this._metadataCacheTime)/1000).toFixed(1)}s)`);
        ...
        try {
            ...
            if (this.liveState === LiveState.ONLINE) {
                ...
                if (this._stableCycles > 3) {
                    const newInterval = Math.min(90000, this.intervalMs + 5000);
