// monitor/liveMonitor.spawn.js
// Versão baseada em spawn (sem maxBuffer, processamento em stream)
...
class LiveMonitor {
    ...
    async getLiveMetadata() {
        try {
            ...
            const isLiveEnded = errorMsg.includes('video unavailable') || errorMsg.includes('not available') || errorMsg.includes('recording is not available');
            
            if (isLiveEnded) this.updateHealthComponent('metadata', ComponentStatus.CRITICAL, `Live encerrada`);
            ...
            } else if (errorMsg.includes('timeout')) this.updateHealthComponent('metadata', ComponentStatus.WARNING, `Timeout`);
            else this.updateHealthComponent('metadata', ComponentStatus.WARNING, `Erro: ${error.message}`);
    ...
    async extractMediaSequence(m3u8Url) {
        ...
        if (typeof currentSequence === 'number' && typeof this.lastMediaSequence === 'number') {
            if (currentSequence > this.lastMediaSequence) {
                ...
                // 🔥 IGNORAR SEQUENCE 0 (playlist master) – NÃO ALTERA HEALTH
        ...
        if (currentSequence === this.lastMediaSequence) {
            if (currentSequence === 0) {
                console.log(`[${this.videoId}] ℹ️ MEDIA-SEQUENCE = 0 (playlist master) – ignorando`);
                return true;
            }
            this.stalledCount++;
            if (this.stalledCount >= this.maxSegmentRepeats) {
    ...
    applyDerivedState() {
        ...
        if(this.liveState !== newState) {
            ...
            if(newState === LiveState.ENDED) { console.log(`[${this.videoId}] 🛑 Live encerrada, parando monitor`); this._liveEnded = true; }
        }
    ...
    updateCache(state, additionalData = {}) {
        ...
        if(this._injectedCache && this._injectedCache instanceof Map) {
            for(const [url, data] of this._injectedCache) {
            ...
            if(!updated) this._injectedCache.set(this.youtubeUrl, { videoId: this.videoId, status: state, isLive: state===LiveState.ONLINE, monitor: this, lastUpdate: new Date(), ...additionalData });
        }
        if(!updated && global.converter && global.converter.liveCache) {
            for(const [url, data] of global.converter.liveCache) {
                if(data.videoId === this.videoId || data.monitor === this) {
                    data.isLive = state === LiveState.ONLINE;
                    data.status = state;
                    data.lastUpdate = new Date();
            ...
            if(!updated) global.converter.liveCache.set(this.youtubeUrl, { videoId: this.videoId, status: state, isLive: state===LiveState.ONLINE, monitor: this, lastUpdate: new Date(), ...additionalData });
        }
        if(!updated) console.warn(`[${this.videoId}] ⚠️ NENHUM cache disponível!`);
    }
    ...
    async checkAndRenew() {
        ...
        try {
            ...
            const metaRes = await this.getLiveMetadata();
            if(!metaRes.success) {
                if(metaRes.isLiveEnded) { this.updateHealthComponent('metadata',ComponentStatus.CRITICAL,'Live encerrada'); this.applyDerivedState(); return; }
                this.updateNetworkHealth(true);
                this.applyDerivedState(); return;
            }
            this.updateNetworkHealth(true);
            const metadata = metaRes.metadata;
            const isValid = this.validateMetadata(metadata);
            if(isValid === false) { this.applyDerivedState(); return; }
            const newUrl = this.extractHlsUrl(metadata);
            if(!newUrl) {
                this.urlFails++;
                this.updateHealthComponent('playlist',ComponentStatus.WARNING,'URL não encontrada');
                if(this.urlFails>=this.maxFails) this.updateHealthComponent('playlist',ComponentStatus.ERROR,`${this.urlFails} falhas`);
                this.applyDerivedState(); return;
            }
            this.urlFails=0;
            if(newUrl !== this.m3u8Url) { this.m3u8Url = newUrl; console.log(`[${this.videoId}] ✅ URL HLS atualizada (spawn)`); }
            const advancing = await this.checkPlaylistProgress(this.m3u8Url);
            if(!advancing) {
                const stillLive = (metadata.live_status === 'is_live' || metadata.is_live === true);
                if(stillLive) { this.updateHealthComponent('playlist',ComponentStatus.WARNING,'Playlist congelada temporariamente'); this.liveState=LiveState.DEGRADED; this.applyDerivedState(); return; }
                this.segmentFails++;
                if(this.segmentFails>=this.maxFails) this.updateHealthComponent('playlist',ComponentStatus.ERROR,`${this.segmentFails} falhas`);
                this.applyDerivedState(); return;
            }
            this.segmentFails=0;
            this.consecutiveUnknownFails=0;
            ...
            console.log(`[${this.videoId}] ✅ Estado: ${this.liveState} | Health:`, { network: this.health.network.status, metadata: this.health.metadata.status, playlist: this.health.playlist.status, cookies: this.health.cookies.status });
        } catch(err) { console.error(`[${this.videoId}] ❌ Erro inesperado (spawn): ${err.message}`); this.applyDerivedState(); }
        finally { this._running=false; if(!this._monitorStopped && !this._liveEnded) this.scheduleNext(); }
    }

    scheduleNext() { if(this.timeoutId) clearTimeout(this.timeoutId); this.timeoutId = setTimeout(()=>this.checkAndRenew(), this.intervalMs); }

    checkCookieRedundancy() {
        const main = path.join(this.cookiesDir,'main.txt'), backup = path.join(this.cookiesDir,'backup.txt'), MIN=5000;
        const mainValid = fs.existsSync(main) && fs.statSync(main).size>MIN;
        const backupValid = fs.existsSync(backup) && fs.statSync(backup).size>MIN;
        if(mainValid && backupValid) this.updateHealthComponent('cookies',ComponentStatus.OK,'Principal e backup OK');
        else if(!mainValid && backupValid) this.updateHealthComponent('cookies',ComponentStatus.WARNING,'Apenas backup disponível');
        else if(!mainValid && !backupValid) this.updateHealthComponent('cookies',ComponentStatus.ERROR,'Nenhum cookie válido');
        const liveCount = global.converter?.liveCache?.size || 0;
        if(!mainValid && !backupValid && !this._criticalSent) { if(this.emailAlerts) this.emailAlerts.sendCriticalAlert(liveCount); this._criticalSent=true; }
        else if(!mainValid && backupValid && !this._failoverSent) { if(this.emailAlerts) this.emailAlerts.sendFailoverAlert(liveCount); this._failoverSent=true; }
    }

    startMonitoring(intervalSeconds=30) {
        this.intervalMs = intervalSeconds*1000;
        this.maxSegmentRepeats = this.calculateMaxRepeats();
        this._monitorStopped=false; this._liveEnded=false;
        console.log(`🔄 Monitor iniciado para ${this.videoId} (intervalo: ${intervalSeconds}s, stallTimeout: ${this.maxStallTimeMs/1000}s, maxRepeats: ${this.maxSegmentRepeats}) [SPAWN VERSION]`);
        console.log(`[${this.videoId}] 🔍 Cache injetado: ${!!this._injectedCache}`);
        this.checkAndRenew();
    }

    stopMonitoring() { this._monitorStopped=true; if(this.timeoutId) clearTimeout(this.timeoutId); console.log(`⏹️ Monitor parado para ${this.videoId} (spawn)`); this.updateCache(this.liveState); }
}
