...
const livePlaylistTtlLogState = new Map();
...
const LIVE_PLAYLIST_TTL_CONFIG = createLivePlaylistTtlConfig(process.env, M3U8_CACHE_TTL);

function logLivePlaylistTtl(cacheKey, cacheEntry) {
    if (!LIVE_PLAYLIST_TTL_CONFIG.enabled || !cacheEntry?.isLivePlaylist) return;
    const signature = `${cacheEntry.targetDuration}:${cacheEntry.effectiveTtlMs}`;
    if (livePlaylistTtlLogState.get(cacheKey) === signature) return;
    livePlaylistTtlLogState.set(cacheKey, signature);
    console.log(
        `[HLS-CACHE-TTL] key=${cacheKey} type=media live=true ` +
        `targetDuration=${cacheEntry.targetDuration} configuredTtlMs=${M3U8_CACHE_TTL} ` +
        `effectiveTtlMs=${cacheEntry.effectiveTtlMs}`
    );
}
...
const HLS_EXOMEDIA_SEGMENT_PROXY = String(process.env.HLS_EXOMEDIA_SEGMENT_PROXY || 'false').toLowerCase() !== 'false';
const HLS_LEGACY_VLC_SEGMENT_PROXY = String(process.env.HLS_LEGACY_VLC_SEGMENT_PROXY || 'false').toLowerCase() !== 'false';
...
const PLAYBACK_STUCK_SESSION_FIX = String(process.env.PLAYBACK_STUCK_SESSION_FIX || '').toLowerCase() === 'true';
const PLAYBACK_STUCK_MAX_SAME_SEQ_HITS = parsePositiveIntegerEnv('PLAYBACK_STUCK_MAX_SAME_SEQ_HITS', 3);
const PLAYBACK_STUCK_MAX_AGE_MS = parsePositiveIntegerEnv('PLAYBACK_STUCK_MAX_AGE_MS', 6000);
...
const sessionPlaylistStuckGuard = new SessionPlaylistStuckGuard({
    enabled: PLAYBACK_STUCK_SESSION_FIX,
    maxSameSeqHits: PLAYBACK_STUCK_MAX_SAME_SEQ_HITS,
    maxAgeMs: PLAYBACK_STUCK_MAX_AGE_MS
});
...
const hlsRecoveredSessionAliases = new Map();

function resolveRecoveredSessionAlias(sessionId, { owner, videoId, tokenScope }, now = Date.now()) {
    const alias = hlsRecoveredSessionAliases.get(String(sessionId || ''));
    if (!alias) return null;
    if (alias.expiresAt <= now || alias.owner !== owner || alias.videoId !== videoId || alias.tokenScope !== tokenScope) {
        hlsRecoveredSessionAliases.delete(String(sessionId || ''));
        return null;
    }
    alias.expiresAt = now + playbackSessions.ttlMs;
    return alias.sessionId;
}

function rememberRecoveredSessionAlias(aliasSessionId, session, tokenScope, now = Date.now()) {
    if (!aliasSessionId || !session?.sessionId) return;
    hlsRecoveredSessionAliases.set(String(aliasSessionId), {
        sessionId: session.sessionId,
        owner: session.owner,
        videoId: session.videoId,
        tokenScope,
        expiresAt: now + playbackSessions.ttlMs
    });
}

function pruneRecoveredSessionAliases(now = Date.now()) {
    for (const [aliasSessionId, alias] of hlsRecoveredSessionAliases.entries()) {
        if (!alias || alias.expiresAt <= now) hlsRecoveredSessionAliases.delete(aliasSessionId);
    }
}
...
function clearSessionVariantState(stateKey) {
    ...
    if (typeof sessionPlaylistStuckGuard !== 'undefined') {
        sessionPlaylistStuckGuard.clear(stateKey);
    }
}
...
function pruneHlsSessionVariantState(now = Date.now()) {
    ...
    if (typeof sessionPlaylistStuckGuard !== 'undefined') {
        sessionPlaylistStuckGuard.prune(maxAgeMs, now);
    }
    return removed;
}
...
function clearHlsSessionVariantStateFor({ owner = null, videoId = null, token = null, sessionId = null } = {}) {
    ...
    for (const [key, state] of hlsSessionVariantState.entries()) {
        if (matches(state)) {
            ...
            hlsSessionVariantPins.delete(key);
            if (typeof sessionPlaylistStuckGuard !== 'undefined') {
                sessionPlaylistStuckGuard.clear(key);
            }
            removed += 1;
        }
    }
...
async function fetchM3u8WithCache(videoId, url, monitorSequence = null) {
    ...
    const cacheTtlMs = cached
        ? getEffectivePlaylistCacheTtl(cached, LIVE_PLAYLIST_TTL_CONFIG)
        : M3U8_CACHE_TTL;
    if (cached && cached.sourceUrl === url && (now - cached.fetchedAt) < cacheTtlMs) {
        ...
        try {
            ...
            const request = protocol.get(url, {
                ...
                res.on('end', () => {
                    ...
                    const cacheEntry = decoratePlaylistCacheEntry({
                        content: body,
                        fetchedAt: Date.now(),
                        sourceUrl: url,
                        mediaSequence: playlistInfo.sequence,
                        lastSequence: playlistInfo.lastSequence
                    }, body, LIVE_PLAYLIST_TTL_CONFIG);
                    m3u8CacheContent.set(videoId, cacheEntry);
                    logLivePlaylistTtl(videoId, cacheEntry);
                    m3u8CachePromises.delete(videoId);
...
async function handleM3u8Proxy(videoId, owner, req, res, maxHeight, routeContext = {}) {
    ...
    if (trackingOwner) {
        ...
        if (!isLocalIp(clientIp)) {
            if (sessionIdFromRequest) {
                const resolvedSessionId = resolveRecoveredSessionAlias(sessionIdFromRequest, {
                    owner: trackingOwner,
                    videoId,
                ...
                if (!touched.ok) {
                    const recoverableSessionLoss = touched.code === 'expired' || touched.code === 'not_found';
                    if (recoverableSessionLoss) {
                        const deviceLimit = getDeviceLimitForOwner(trackingOwner);
                        const recovered = playbackSessions.createSession({
                            owner: trackingOwner,
                            videoId,
                            tokenScope: playbackSessionTokenScope,
                            limit: deviceLimit,
                            publicIp: clientIp,
                            userAgent,
                            source: 'hls-session-recovery',
                            fingerprint: localIp ? `localIp:${localIp}` : null
                        });
                        if (!recovered.ok) return sendHlsError(res, 429, 'limit_exceeded');
                        rememberRecoveredSessionAlias(sessionIdFromRequest, recovered.session, playbackSessionTokenScope);
                        activePlaybackSessionId = recovered.session.sessionId;
                        activePlaybackSession = recovered.session;
                        console.log(`[${trackingOwner}:${videoId}] sessao HLS recuperada session=${sessionPreview(activePlaybackSessionId)}`);
                    } else {
                        console.warn(`[${trackingOwner}:${videoId}] sessao HLS rejeitada (${touched.code}) session=${sessionPreview(sessionIdFromRequest)}`);
                        return sendHlsError(res, 403, 'session_invalid');
                    }
                } else {
                    activePlaybackSessionId = resolvedSessionId;
    ...
    if (effectiveUrlMaxHeight && monitor._playlistUrls && monitor._playlistUrls[effectiveUrlMaxHeight]) {
        ...
        const fetchVariantPlaylist = async (sourceUrl, source, cacheKeyOverride = null) => {
            const upstreamHash = getUpstreamIdentityHash(sourceUrl);
            const variantCacheKey = cacheKeyOverride || `${cacheKey}_${upstreamHash}`;
        ...
        try {
            ...
            try {
                ...
                const stuckObservation = sessionPlaylistStuckGuard.observe(
                    pinKey,
                    variant.snapshot?.mediaSequence
                );
                if (pinKey && playlistUrl && stuckObservation.shouldProbe) {
                    sessionPlaylistStuckGuard.markProbe(pinKey);
                    const stuckProbeCacheKey = `${cacheKey}_stuck_probe_${getShortHash(pinKey, 16)}`;
                    m3u8CacheContent.delete(stuckProbeCacheKey);
                    m3u8CachePromises.delete(stuckProbeCacheKey);
                    try {
                        const refreshed = await fetchVariantPlaylist(
                            playlistUrl,
                            'session-stuck-refresh',
                            stuckProbeCacheKey
                        );
                        const currentSequence = Number(variant.snapshot?.mediaSequence);
                        const newerSequence = Number(refreshed.snapshot?.mediaSequence);
                        if (shouldRepinSessionPlaylist(
                            currentSequence,
                            newerSequence,
                            playlistsHaveOverlap(variant.snapshot, refreshed.snapshot)
                        )) {
                            const oldUpstream = getUpstreamIdentityHash(playlistSourceUrl);
                            const mainVariantCacheKey = `${cacheKey}_${getUpstreamIdentityHash(refreshed.sourceUrl)}`;
                            const promotedMainCache = promoteValidatedPlaylistCache({
                                cache: m3u8CacheContent,
                                cacheKey: mainVariantCacheKey,
                                sourceUrl: refreshed.sourceUrl,
                                content: refreshed.result.content,
                                snapshot: refreshed.snapshot
                            });
                            if (!promotedMainCache) {
                                throw new Error('invalid_playlist_cache_promotion');
                            }
                            decoratePlaylistCacheEntry(
                                m3u8CacheContent.get(mainVariantCacheKey),
                                refreshed.result.content,
                                LIVE_PLAYLIST_TTL_CONFIG
                            );
                            m3u8CachePromises.delete(mainVariantCacheKey);
                            playlistSourceUrl = refreshed.sourceUrl;
                            variant = refreshed;
                            clearPinnedVariantUrl(pinKey);
                            hlsSessionVariantPins.delete(pinKey);
                            sessionPlaylistStuckGuard.markRecovered(pinKey, newerSequence);
                            console.log(
                                `[HLS-STUCK-FIX] videoId=${videoId} owner=${trackingOwner || 'n/a'} ` +
                                `session=${sessionPreview(activePlaybackSessionId)} q=${urlMaxHeight} ` +
                                `seq=${currentSequence} sameHits=${stuckObservation.sameHits} ` +
                                `ageMs=${stuckObservation.ageMs} newerSeq=${newerSequence} ` +
                                `oldUpstream=${oldUpstream} action=repin promoteMainCache=true`
                            );
                        }
                    } catch (stuckProbeError) {
                        console.warn(
                            `[HLS-STUCK-FIX] videoId=${videoId} q=${urlMaxHeight} ` +
                            `session=${sessionPreview(activePlaybackSessionId)} action=probe-failed ` +
                            `error=${String(stuckProbeError?.code || stuckProbeError?.statusCode || 'upstream_error')}`
                        );
                    } finally {
                        m3u8CacheContent.delete(stuckProbeCacheKey);
                        m3u8CachePromises.delete(stuckProbeCacheKey);
